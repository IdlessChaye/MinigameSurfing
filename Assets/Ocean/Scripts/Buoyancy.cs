...
public class Buoyancy : MonoBehaviour
{
	...
	public bool isSyncWithSea = false;
	public bool isBuoyancy = false;
	...
	void Start()
	{
		...
		{
			if (cvisible)
			{ Debug.Log("Renderer to check visibility not assigned."); cvisible = false; }
			if (wvisible)
			{ Debug.Log("Renderer to check visibility not assigned."); wvisible = false; }
			if (svisible)
			{ Debug.Log("Renderer to check visibility not assigned."); svisible = false; }
		}

		if (dampCoeff < 0)
			dampCoeff = Mathf.Abs(dampCoeff);
		...
		_rigidbody.centerOfMass = new Vector3(0.0f, CenterOfMassOffset, 0.0f);
	...
	void update()
	{
		...
		if (ocean != null)
		{
			...
			if (interpolate)
			{ tick++; if (tick == intplt) tick = 0; }

			tack++;
			if (tack == (int)Random.Range(2, 9))
				tack = 0;
			if (tack > 9)
				tack = 1;
			...
			float fact = _rigidbody.velocity.magnitude * 0.02f;
				...
				if (ChoppynessAffectsPosition)
				{
					if (!cvisible || visible)
					{
						if (ocean.choppy_scale > 0)
						{
							...
							{
								if (tack == 0)
									_rigidbody.AddForceAtPosition(-Vector3.left * (ocean.GetChoppyAtLocation2Fast() * ChoppynessFactor * Random.Range(0.5f, 1.3f)) * fact2, cpos);
								else
									_rigidbody.AddForceAtPosition(-Vector3.left * (ocean.GetChoppyAtLocation2Fast() * ChoppynessFactor * Random.Range(0.5f, 1.3f)) * fact2, _transform.position);
							}
							else
							{
								if (tack == 0)
									_rigidbody.AddForceAtPosition(-Vector3.left * (ocean.GetChoppyAtLocationFast() * ChoppynessFactor * Random.Range(0.5f, 1.3f)) * fact2, cpos);
								else
									_rigidbody.AddForceAtPosition(-Vector3.left * (ocean.GetChoppyAtLocationFast() * ChoppynessFactor * Random.Range(0.5f, 1.3f)) * fact2, _transform.position);
							}
						}
					}
				}
				...
				if (WindAffectsPosition)
				{
					if (!wvisible || visible)
					{
						...
						if (tack == 1)
							...
							_rigidbody.AddForceAtPosition(new Vector3(ocean.pWindx, 0, ocean.pWindy) * WindFactor * fact2, _transform.position);
					}
				}
			}
			...
			if (xAngleAddsSliding)
			{
				if (!svisible || visible)
				{
					float xangle = _transform.localRotation.eulerAngles.x;
					...
					if (accel > 0)
					{ accel -= 0.05f; if (accel < 0) accel = 0; }
					if (accel < 0)
					{ accel += 0.05f; if (accel > 0) accel = 0; }
				}
			}
