...
namespace NoMono
{
	public class GameManager : FullSingleton<GameManager>
	{
		...
		private LinkedList<BaseRoundManager> _roundManagerLinList = new LinkedList<BaseRoundManager>();
		...
		private LinkedList<BaseRoundManager> _tickRoundManagerLinList = new LinkedList<BaseRoundManager>();
		...
		private List<BaseRoundManager> _toBeAddedRoundList = new List<BaseRoundManager>(); 
		...
		private void AddLocalManagers(Scene scene)
		{
			...
			else if (sceneName.Equals("TestCombatPage"))
			{
				AddLocalManager(new UIManager());
			}
			else if (sceneName.Equals("TestCombatTabPage"))
			{
				AddLocalManager(new UIManager());
			}
			else if (sceneName.Equals("WuXingTestScene 1"))
			{
				AddLocalManager(new UIManager());
			} else if (sceneName.Equals("xjj_MoveWallTest"))
			{
				AddLocalManager(new UIManager());
			}
			  //else if (sceneName.Equals("TestSwordSkillPage"))
			  //{
			  //	AddLocalManager(new UIManager());
			  //}
			  else if (sceneName.Equals("TestUpgradePage"))
            {
				AddLocalManager(new UIManager());
            } 
			else if (sceneName.Equals("TestWuxingUpgradePage"))
            {
				AddLocalManager(new UIManager());
            } 
			else if (sceneName.Equals("WuXingTestScene_hhm"))
            {
				AddLocalManager(new UIManager());
            } 
			else if(sceneName.Equals("TestSwordStatusPage"))
            {
				AddLocalManager(new UIManager());
			} 
			else if(sceneName.Equals("lhbTestScene"))
            {
				AddLocalManager(new UIManager());
			}
			else if (sceneName.Equals("SampleScene"))
			{
				...
				//AddLocalManager(new GachaManager());
			}
			else if (sceneName.Equals("hhm_UITest"))
            {
				AddLocalManager(new UIManager());
            } else if (sceneName.Equals("hhm_CombatPageTest"))
            {
				AddLocalManager(new UIManager());
            }
		...
		private void OnSceneLoaded(Scene scene, LoadSceneMode loadSceneMode)
		{
			...
			OnRoundManagerLoaded(scene, loadSceneMode);
		}
		...
		private void OnGlobalManagerLoaded(Scene scene, LoadSceneMode loadSceneMode)
		{
			...
			if (_toBeAddedGlobalList.Count != 0)
			{
				...
				{
					manager = _toBeAddedGlobalList[i];
					if (manager != null)
					{
						manager.TickAddTo();
		...
		private void OnRoundManagerLoaded(Scene scene, LoadSceneMode loadSceneMode)
        {
			var node = _roundManagerLinList.First;
			BaseRoundManager manager = null;
			while (node != null)
			{
				manager = node.Value;
				if (manager != null)
			...
			if (_toBeAddedRoundList.Count != 0)
			{
				for (int i = 0; i < _toBeAddedRoundList.Count; i++)
				{
					manager = _toBeAddedRoundList[i];
					if (manager != null)
					{
						manager.TickAddTo();

						_roundManagerLinList.AddLast(manager);
						_managerDict.Add((uint)manager.managerType, manager);
						if (manager.needTick)
							_tickRoundManagerLinList.AddLast(manager);
					}
				}
		...
		private void OnRoundManagerUnloaded(Scene scene)
        {
			var node = _roundManagerLinList.Last;
			BaseRoundManager manager = null;
			while (node != null)
			{
				manager = node.Value;
		...
		private void RemoveRoundManagers()
        {
			var node = _roundManagerLinList.Last;
			BaseRoundManager manager = null;
			while (node != null)
			...
			_roundManagerLinList.Clear();
			_tickRoundManagerLinList.Clear();
			_toBeAddedRoundList.Clear();
		}
		...
		public void AddGlobalManager(BaseGlobalManager manager)
		{
			...
			isDirtyAdd = true;
			manager.Init();
			if (!_toBeAddedGlobalList.Contains(manager))
		...
		public void AddRoundManager(BaseRoundManager manager)
        {
			if (manager == null)
				return;
			if (_managerDict.ContainsKey((uint)manager.managerType))
			...
			isDirtyAdd = true;
			manager.Init();
            if (!_toBeAddedRoundList.Contains(manager))
                _toBeAddedRoundList.Add(manager);
        }
		...
		private void Update()
		{
			...
			#region Tick round Manager

			var nodeRound = _tickRoundManagerLinList.First;
			BaseRoundManager managerRound = null;
			while (nodeRound != null)
			{
				managerRound = nodeRound.Value;
				if (managerRound != null && managerRound.needTick) // manager 的 isActive 一直都是 true，如果不是，则不是Manager，而是 Entity
				{
					managerRound.Tick0(deltaTime);
				}
				nodeRound = nodeRound.Next;
			}

			nodeRound = _tickRoundManagerLinList.First;
			managerRound = null;
			while (nodeRound != null)
			{
				managerRound = nodeRound.Value;
		...
		private void LateUpdate()
		{
			...
			if (isDirtyAdd == true)
			{
				...
				var nodeRound = _roundManagerLinList.First;
				BaseRoundManager managerRound = null;
				while (nodeRound != null)
				{
					managerRound = nodeRound.Value;
					if (managerRound != null && managerRound.isDirtyAdd)
					{
						managerRound.TickAddTo();
					}
					nodeRound = nodeRound.Next;
				}
				...
				if (_toBeAddedRoundList.Count != 0)
				{
					for (int i = 0; i < _toBeAddedRoundList.Count; i++)
					{
						...
						if (managerRound != null)
						{
							managerRound.TickAddTo();

							_roundManagerLinList.AddLast(managerRound);
							_managerDict.Add((uint)managerGlobal.managerType, managerRound);
							if (managerRound.needTick)
								_tickRoundManagerLinList.AddLast(managerRound);
						}
					}
